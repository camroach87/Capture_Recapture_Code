...
CR_RobustDesign <- function(window, sim="N", N.0=1000, nsampOcc=14, p=0.05, phi=0.9, pBirth=0.1, pImmigration=0.1) {
  ...
  aCaptWindow <- array(0,c(nrow(mtrxCapt),window+1,nsampOcc-window))
  aChao <- array(0,c(nsampOcc-window,2))
  
  for (i in 1:(nsampOcc-window)) {
    aCaptWindow[,1:(1+window),i] <- mtrxCapt[,i:(i+window)]
